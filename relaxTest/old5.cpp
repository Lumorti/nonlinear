...
double f(Eigen::VectorXd Cyz, Eigen::SparseMatrix<double> WPlus, double x_lambda, Eigen::VectorXd y) {

	// Calculate and return the object function
	return fScaling*0.5*Cyz.transpose()*WPlus*Cyz + x_lambda*numMats + y.transpose()*b;

}

// Derivative of the objective function
Eigen::VectorXd delf(Eigen::VectorXd Cyz, Eigen::SparseMatrix<double> WPlus) {
...
Eigen::VectorXd g(Eigen::VectorXd Cyz, Eigen::SparseMatrix<double> P) {

	// Create an empty vector
	Eigen::VectorXd returnVec(m);

	// This is only zero if the (y^TC-z^T) is in the row space of Q 
	returnVec(0) = (P*Cyz-Cyz).squaredNorm();
...
Eigen::MatrixXd delg(Eigen::VectorXd Cyz, Eigen::SparseMatrix<double> P) {

	// Create an empty matrix
	Eigen::MatrixXd returnMat(m, n);

	// Each section has a slightly different derivative
	returnMat(0,0) = ((identityQ-P)*Cyz).squaredNorm();
...
// The Lagrangian 
//double L(Eigen::VectorXd Cyz, Eigen::SparseMatrix<double> P, Eigen::SparseMatrix<double> WPlus, Eigen::SparseMatrix<double> X, Eigen::VectorXd y, Eigen::SparseMatrix<double> Z) {
	//return std::real(f(Cyz, WPlus) - y.dot(g(Cyz, P)) - X.cwiseProduct(Z).sum());
//}
...
double F(Eigen::VectorXd x, Eigen::SparseMatrix<double> Z, double mu) {
	...
	// Extract some stuff
	double x_lambda = x(0);
	Eigen::VectorXd x_y = x.segment(1, numy);
	Eigen::VectorXd x_z = x.tail(numz);

	// Calculate some useful matrices TODO
	Eigen::SparseMatrix<double> W = Q + x_lambda*identityQ;
	Eigen::SparseMatrix<double> WPlus = pseudo(W);
	Eigen::SparseMatrix<double> P = W*WPlus;
	Eigen::VectorXd Cyz = C.transpose()*x_y-x_z;

	// Cache the X matrix
	Eigen::SparseMatrix<double> X = vecToMat(x);
	Eigen::VectorXd gCached = g(Cyz, P);
	...
	double FBP = std::real(f(Cyz, WPlus, x_lambda, x_y) - mu*std::log(XDeter) + rho*gCached.norm());
...
int main(int argc, char ** argv) {
	...
	double minEigen = Eigen::MatrixXd(Q).eigenvalues().real().minCoeff();

	// Top left element is the lowest eigenvalue of Q
	tripsE.push_back(Eigen::Triplet<double>(0, 0, minEigen-extraDiag));
	...
	//for (int i=1; i<p; i+=2*d) {
		//Eigen::VectorXcd rand = Eigen::VectorXcd::Random(d).normalized();
	...
	// Init X as the dense matrix
	XDense = Eigen::MatrixXd::Identity(p, p);

	// Then turn this into an x
	X = XDense.sparseView();
	x = matToVec(X);
	x(0) = -minEigen*2;
	x.segment(1, numy) = Eigen::VectorXd::Zero(numy);
	...
	Eigen::SparseMatrix<double> P = W*WPlus;
	Eigen::VectorXd Cyz = C.transpose()*x_y-x_z;
	N = Eigen::VectorXd::Constant(1, numMats);
	...
	Eigen::VectorXd gCached = g(Cyz, P);
	Eigen::MatrixXd A_0 = delg(Cyz, P);
	Eigen::VectorXd delfCached = delf(Cyz, WPlus);
	...
	for (k=0; k<maxOuterIter; k++) {
		...
		for (int k2=0; k2<maxInnerIter; k2++) {
			...
			// Extract the vars from the x
			x_lambda = x(0);
			x_y = x.segment(1, numy);
			x_z = x.tail(numz);

			// Calculate some useful matrices TODO
			W = Q + x_lambda*identityQ;
			WPlus = pseudo(W);
			P = W*WPlus;
			Cyz = C.transpose()*x_y-x_z;
			...
			gCached = g(Cyz, P);
			A_0 = delg(Cyz, P);
			delfCached = delf(Cyz, WPlus);
			delLCached = delL(y, Z, delfCached, A_0);

			// TODO nans
			//prettyPrint("x = ", x);
			//prettyPrint("X = ", X);
			//prettyPrint("XInverse = ", XInverse);
			...
			if (outputMode == "") {
				std::cout << std::scientific << "f=" << f(Cyz, WPlus, x_lambda, x_y) << " r=" << rMagMu  << " g=" << gCached.norm() << " lam=" << x(0) << std::endl;
			}
	...
	for (int i=0; i<numRhoMats; i++) {
		...
		primalx(i*4+1) = Rs[i].real()(0, 1);
		primalx(i*4+2) = Rs[i].real()(1, 1);
		primalx(i*4+3) = Rs[i].imag()(0, 1);
	}
	for (int i=0; i<numBMats; i++) {
		primalx(numRhoMats*4+i*4+0) = std::real(Bs[i][0][0]);
		primalx(numRhoMats*4+i*4+1) = std::real(Bs[i][0][1]);
		primalx(numRhoMats*4+i*4+2) = std::real(Bs[i][1][1]);
		primalx(numRhoMats*4+i*4+3) = std::imag(Bs[i][0][1]);
	}
	...
	// Output the final main vector
	std::cout << std::endl;
	std::cout << "----------------------------------" << std::endl;
	std::cout << "         Final Matrices " << std::endl;;
	std::cout << "----------------------------------" << std::endl;
	prettyPrint("final x = ", x);
	prettyPrint("min eigen of Q = ", minEigen);

	// Final output
	if (outputMode == "") {
		double finalf = f(Cyz, WPlus, x_lambda, x_y);
		std::cout << std::scientific << "" << std::endl;
		std::cout << "----------------------------------" << std::endl;
		std::cout << "         Final Output " << std::endl;;
		std::cout << "----------------------------------" << std::endl;
		std::cout << "        |r(w)| = " << rMagZero << " < " << epsilon << std::endl;;
		std::cout << "          f(x) = " << finalf << " >= " << maxVal << std::fixed << std::setprecision(2) << " (" << 100*(finalf-maxVal)/maxVal << std::scientific << std::setprecision(precision) << "\% higher)" << std::endl;;
		std::cout << "        |g(x)| = " << gCached.norm() << std::endl;;
		//std::cout << "          L(w) = " << L(Cyz, P, WPlus, X, y, Z) << std::endl;;
		std::cout << "         <X,Z> = " << X.cwiseProduct(Z).sum() << std::endl;;
		std::cout << "           |y| = " << y.norm() << std::endl;;
		std::cout << "      y^T*g(x) = " << y.transpose()*g(Cyz, P) << std::endl;;
